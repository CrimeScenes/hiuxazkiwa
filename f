...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = getgenv().TriggerBot.Settings.BoxFov.Height * 100
    local fovWidth = getgenv().TriggerBot.Settings.BoxFov.Width * 100
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            if getgenv().TriggerBot.Settings.Preds.LegMode then
                predictedPosition = getPredictedPosition(TargetPlayer.Character)
            else
                predictedPosition = TargetPlayer.Character.HumanoidRootPart.Position
            end

            if predictedPosition and isWithinBoxFOV(predictedPosition) then
                local closestPart = findClosestPart(TargetPlayer.Character)
                if closestPart then
                    local distance = (Camera.CFrame.Position - closestPart.Position).Magnitude
                    if distance <= getgenv().TriggerBot.Settings.TriggerDistance then
                        if os.clock() - lastClickTime >= getgenv().TriggerBot.Settings.MinDelay then
